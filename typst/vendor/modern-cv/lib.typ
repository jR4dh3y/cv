...
#let resume(
  ...
  // Accent-tinted icons for the contact column
  let contact-icon(name) = box(
    baseline: 0.5pt,
    fa-icon(name, fill: accent-color, size: 8pt),
  )

  // Strip scheme/trailing slash so URLs read cleanly in the header
  let display-url(url) = {
    let s = str(url)
    if s.starts-with("https://") {
      s = s.slice(8)
    } else if s.starts-with("http://") {
      s = s.slice(7)
    }
    if s.ends-with("/") {
      s = s.slice(0, -1)
  ...
  let name = {
    set text(size: 28pt, style: "normal", font: header-font, tracking: -0.4pt)
    if language == "zh" or language == "ja" [
      ...
      #text(weight: "medium", fill: color-darknight)[#author.firstname]
      #h(0.12em)
      #text(weight: "bold", fill: accent-color)[#author.lastname]
    ]
  }

  // Optional 1–2 line summary under the accent rule (author.summary)
  let summary = {
    if ("summary" in author) {
      set text(size: 9.5pt, weight: "regular", fill: color-darkgray, style: "normal")
      set par(leading: 0.5em, justify: false)
      author.summary
    }
  }

  // Right-side contact stack (fills space beside the name)
  let contact-rows = ()
  if ("phone" in author) {
  ...
  if ("gitlab" in author) {
    contact-rows.push(contact-row(
      "gitlab",
      link("https://gitlab.com/" + author.gitlab)[#author.gitlab],
    ))
  }
  if ("bitbucket" in author) {
    contact-rows.push(contact-row(
      "bitbucket",
      link("https://bitbucket.org/" + author.bitbucket)[#author.bitbucket],
    ))
  }
  ...
  if ("twitter" in author) {
    contact-rows.push(contact-row(
      "twitter",
      link("https://twitter.com/" + author.twitter)[\@#author.twitter],
    ))
  }
  if ("scholar" in author) {
    ...
    contact-rows.push(contact-row(
      "google-scholar",
      link("https://scholar.google.com/citations?user=" + author.scholar)[#fullname],
    ))
  }
  if ("orcid" in author) {
    contact-rows.push(contact-row(
  ...
  if ("website" in author) {
    contact-rows.push(contact-row(
      "globe",
      link(author.website)[#display-url(author.website)],
    ))
  }
  if ("birth" in author) {
    contact-rows.push(contact-row("cake", text(author.birth)))
  }
  ...
  if ("custom" in author and type(author.custom) == array) {
    for item in author.custom {
      if ("text" in item) {
        let icon = if ("icon" in item) { item.icon } else { "link" }
        let body = if ("link" in item) {
          link(item.link)[#item.text]
        } else {
          item.text
        }
        contact-rows.push(contact-row(icon, body))
      }
    }
  }
  ...
  let contacts = {
    ...
    contact-rows.join(v(3.5pt, weak: true))
  }

  // Two-column header:
  //   name + rule + summary (left)  |  contacts (right)
  //   rule runs under the name up to the contact column
  let header-block = {
    grid(
      columns: (1fr, auto),
      column-gutter: 16pt,
